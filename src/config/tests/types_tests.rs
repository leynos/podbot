...
fn app_config_toml_sets_engine_socket_and_image(
    #[from(app_config_from_full_toml)] parsed: Result<AppConfig, toml::de::Error>,
) {
    let config = parsed.expect("the full TOML example parses");
    assert_eq!(
...
fn app_config_partial_toml_sets_engine_socket(
    #[from(app_config_from_partial_toml)] parsed: Result<AppConfig, toml::de::Error>,
) {
    let config = parsed.expect("the minimal TOML example parses");
    assert_eq!(
        config.engine_socket.as_deref(),
