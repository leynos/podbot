...
/// Maximum bytes buffered between container input write operations. This
/// complements `STDIN_BUFFER_CAPACITY` to ensure the write side also remains
/// explicitly bounded, preventing the copy operation from accumulating an
/// unbounded write queue. The value matches the read buffer size to maintain
/// symmetrical backpressure behaviour.
const OUTPUT_BUFFER_CAPACITY: usize = 65_536;
    ...
    HostStdin: AsyncRead + Unpin,
{
    ...
    let mut buffered_input = tokio::io::BufWriter::with_capacity(OUTPUT_BUFFER_CAPACITY, input);
