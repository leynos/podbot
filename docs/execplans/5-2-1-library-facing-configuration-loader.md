...
Previously, configuration loading (`podbot::config::load_config`) required a
Clap parse struct (`podbot::config::Cli`). That coupling forced library
embedders to depend on CLI argument parsing types and (in practice) on Clap
conventions, which contradicts the dual-delivery model in
...
1. A library consumer can call `podbot::config::load_config(&ConfigLoadOptions)`
   and receive an `AppConfig` using explicit load options and overrides,
   without importing any Clap parse structs.
...
- [x] (2026-03-03) Draft and review this ExecPlan.
- [x] (2026-03-03) Implement library-facing config loader API and refactor
  internals.
- [x] (2026-03-03) Move Clap parse types into a CLI adapter layer and add
  conversion helpers.
- [x] (2026-03-03) Add unit + behavioural tests for library config loading.
- [x] (2026-03-03) Update design doc and roadmap entry (no users' guide changes
  required).
- [x] (2026-03-03) Run gates: `make check-fmt`, `make lint`, `make test`.
...
- `podbot::config` now exposes `ConfigLoadOptions`, `ConfigOverrides`,
  `load_config`, and `load_config_with_env`, with no dependency on Clap parse
  types.
- Clap parse structs are now in `podbot::cli`, and the binary uses
  `Cli::config_load_options()` to convert flags into library load options.
- Added unit, integration, and BDD coverage for library configuration loading
  independent of CLI parsing.
- Updated `docs/podbot-design.md` and marked roadmap Step 5.2 tasks as done.
...
3. Library configuration loading is implemented in `src/config/loader.rs`:
   ...
   - `pub fn load_config_with_env(env: &impl mockable::Env, options: &ConfigLoadOptions)
     -> Result<AppConfig>`
4. The CLI binary (`src/main.rs`) parses `podbot::cli::Cli`, converts it to
...
Implemented library interfaces:

```rust
pub struct ConfigLoadOptions {
    pub config_path_hint: Option<camino::Utf8PathBuf>,
...
pub fn load_config(options: &ConfigLoadOptions) -> crate::error::Result<crate::config::AppConfig>;
```

Deterministic tests use the environment-injected variant:
...
1. Add focused unit tests for `load_config_with_env` verifying:
   - Happy path: overrides are applied (engine socket, image).
   - Unhappy path: invalid typed environment values produce a semantic
     `ConfigError::InvalidValue`.
   - Edge cases: explicit config path missing is ignored and the loader falls
     back to defaults or discovery (matching CLI behaviour).
...
2. Add `tests/bdd_config_loader.rs` and helper code under
   `tests/bdd_config_loader_helpers.rs` following repository conventions.
...
- Files changed:
  - `src/config/load_options.rs`, `src/config/loader.rs`, `src/config/mod.rs`,
    `src/config/types.rs`
  - `src/cli/mod.rs`, `src/cli/tests.rs`, `src/lib.rs`, `src/main.rs`
...
Updated to COMPLETE on 2026-03-03 after implementation and validation.
