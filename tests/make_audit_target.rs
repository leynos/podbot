...
fn run_rust_audit(workspace: &Path, fake_cargo: &Path) -> TestResult<std::process::Output> {
    Ok(Command::new("make")
        .arg("-f")
...
#[test]
fn rust_audit_invokes_cargo_audit_for_each_non_pruned_manifest() {
    ...
    create_manifest(&workspace.join("crates/agent/Cargo.toml"))
    ...
    let output = run_rust_audit(workspace, &fake_cargo).expect("make rust-audit should run");
    ...
    assert!(
        ...
        "target manifests should be pruned: {log}"
    );
    assert!(
        !log.contains("node_modules/ignored"),
        "node_modules manifests should be pruned: {log}"
    );
    assert!(
        !log.contains(".venv/ignored"),
        ".venv manifests should be pruned: {log}"
    );
}
